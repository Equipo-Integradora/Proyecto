...
<?php
extract($_POST);
if ($_POST && !empty($id_venta)) { // Verifica si $_POST tiene datos y si id_venta no está vacío
    $consultaordenes = "SELECT
    id_venta,
    nombre_usuario,
    GROUP_CONCAT(nombre_producto SEPARATOR ', ') AS productos,
    SUM(cantidad_producto_orden_venta) AS cantidad_total, /* Suma la cantidad de productos */
    SUM(precio_producto * cantidad_producto_orden_venta) AS precio_total,
    estado_orden_venta
    FROM sweet_beauty.`todas las ordenes`
    WHERE id_venta LIKE '%$id_venta%' /* No es necesario verificar nuevamente si el id_venta no está vacío porque ya lo hemos hecho arriba */
    GROUP BY id_venta";
        ...
        <table class="table shadow-sm table-hover">
            ...
            if (empty($tablac)) {
                ...
                foreach ($tablac as $reg) {
                    ...
                    echo '<td>';
                    echo '<button class="btn btn-secondary btn-sm" type="button" data-bs-toggle="collapse"
                            data-bs-target="#productos-' . $reg->id_venta . '">Ver productos</button>';
